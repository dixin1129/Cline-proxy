...
func main() {
	...
	if *showList {
		...
		for i, a := range accounts {
			fmt.Printf("  %d. [%s] %s (status: %s, calls: %d today / %d total, tokens: %d today / %d total)\n",
				i+1, a.AccountID, a.Email, a.Status, a.UsageCountToday, a.UsageCount, a.TokensToday, a.TokensTotal)
		}
		fmt.Println()
		return
	}
